...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const existedUser = await User.findOne({
    $or: [{ username }, { email }], // {username: username} {email: email}
  });
  ...
  if (!createdUser) {
    throw new ApiError(500, "Someting went wrong when registering user");
  }
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        { user: createdUser },
        "User registeration successful and verification email sent on your email"
      )
    );
});
...
const resetForgotPassword = asyncHandler(async (req, res) => {
  ...
  user.password = newPassword; // pre hook will automatically hash the password
